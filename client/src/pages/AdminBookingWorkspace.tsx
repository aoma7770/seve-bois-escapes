...
import { CalendarDays, CheckCircle2, Copy, CreditCard, ExternalLink, Mail, MessageSquare, Phone, RefreshCcw, Save, Search, UserRound, XCircle } from "lucide-react";
...
export default function AdminBookingWorkspace() {
  ...
  const createPaymentLink = trpc.admin.createPaymentLink.useMutation({
    onSuccess: (data) => { setPaymentLink(data.url ?? ""); setMessage("Payment link created. The booking remains unconfirmed until Stripe reports payment success."); utils.admin.getBookingProfile.invalidate(); utils.admin.getBookings.invalidate(); },
    onError: (error) => setMessage(error.message),
  });
  ...
  useEffect(() => {
    ...
    setDraft({
      ...
      guestFirstName: booking.guestFirstName || booking.guestName.split(" ")[0] || "",
      guestSurname: booking.guestSurname || booking.guestName.split(" ").slice(1).join(" ") || "",
  ...
  const saveDetails = () => {
    ...
    const paymentChanged = selected && (
      selected.bookingSelection !== draft.bookingSelection ||
      dateValue(selected.checkIn) !== draft.checkIn ||
      dateValue(selected.checkOut) !== draft.checkOut ||
      Number(selected.guestCount) !== Number(draft.guestCount) ||
      Number(selected.totalAmount) !== Number(draft.totalAmount) ||
      Number(selected.cleaningFee) !== Number(draft.cleaningFee)
    );
    updateBooking.mutate({
      id: selectedBookingId,
      bookingSelection: draft.bookingSelection,
      guestFirstName: draft.guestFirstName,
      guestSurname: draft.guestSurname,
      guestName: `${draft.guestFirstName} ${draft.guestSurname}`.trim(),
      ...
      requiresPayment: Boolean(paymentChanged),
    });
  };
