...
export default function TermsPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--cream-50)]" style={{ paddingTop: "4rem" }}>
      <div className="bg-[var(--forest-900)] py-12 text-white"><div className="container"><h1 className="text-headline text-white">{t({ fr: "Conditions générales & de réservation", en: "Terms & Booking Conditions", nl: "Algemene voorwaarden & reserveringsvoorwaarden" })}</h1></div></div>
      <div className="container py-16 max-w-3xl prose-eco">
        <p className="text-caption text-[var(--slate-500)] mb-8">{t({ fr: "Dernière mise à jour : juillet 2025", en: "Last updated: July 2025", nl: "Laatst bijgewerkt: juli 2025" })}</p>
        <h2 className="text-subheadline text-[var(--forest-950)] mb-4">{t({ fr: "1. Réservation", en: "1. Booking", nl: "1. Boeking" })}</h2>
        <p className="text-lead mb-6">{t({ fr: "La réservation est confirmée à réception du paiement intégral. Un email de confirmation vous est envoyé dans les 24h. Le séjour minimum est de 2 nuits.", en: "The booking is confirmed upon receipt of full payment. A confirmation email is sent within 24 hours. The minimum stay is 2 nights.", nl: "De boeking wordt bevestigd na ontvangst van volledige betaling. Een bevestigingsemail wordt binnen 24 uur verzonden. De minimale verblijfsduur is 2 nachten." })}</p>
        <h2 className="text-subheadline text-[var(--forest-950)] mb-4">{t({ fr: "2. Annulation", en: "2. Cancellation", nl: "2. Annulering" })}</h2>
        <p className="text-lead mb-6">{t({ fr: "Annulation gratuite jusqu'à 14 jours avant l'arrivée. Entre 7 et 14 jours : remboursement de 50% du montant total. Moins de 7 jours avant l'arrivée : aucun remboursement. En cas de force majeure, nous étudions chaque situation individuellement.", en: "Free cancellation up to 14 days before arrival. Between 7 and 14 days: 50% refund of total amount. Less than 7 days before arrival: no refund. In case of force majeure, we consider each situation individually.", nl: "Gratis annulering tot 14 dagen voor aankomst. Tussen 7 en 14 dagen: 50% terugbetaling van het totaalbedrag. Minder dan 7 dagen voor aankomst: geen terugbetaling. In geval van overmacht bekijken we elke situatie individueel." })}</p>
        <h2 className="text-subheadline text-[var(--forest-950)] mb-4">{t({ fr: "3. Arrivée & départ", en: "3. Check-in & check-out", nl: "3. Aankomst & vertrek" })}</h2>
        ...
        <h2 className="text-subheadline text-[var(--forest-950)] mb-4">{t({ fr: "4. Règlement intérieur", en: "4. House rules", nl: "4. Huisregels" })}</h2>
        <p className="text-lead mb-6">{t({ fr: "Non-fumeur à l'intérieur. Respect du voisinage et de la nature. Animaux acceptés sur demande. Tout dommage causé au cottage sera facturé au locataire.", en: "Non-smoking indoors. Respect for neighbours and nature. Pets accepted on request. Any damage caused to the cottage will be charged to the tenant.", nl: "Rookvrij binnenshuis. Respect voor buren en natuur. Huisdieren welkom op aanvraag. Eventuele schade aan het cottage wordt in rekening gebracht bij de huurder." })}</p>
        <h2 className="text-subheadline text-[var(--forest-950)] mb-4">{t({ fr: "5. Responsabilité", en: "5. Liability", nl: "5. Aansprakelijkheid" })}</h2>
        <p className="text-lead">{t({ fr: "Sève & Bois Escapes ne peut être tenu responsable des accidents survenus en dehors du cottage. Les locataires sont responsables de leur propre sécurité lors des activités extérieures.", en: "Sève & Bois Escapes cannot be held responsible for accidents occurring outside the cottage. Tenants are responsible for their own safety during outdoor activities.", nl: "Sève & Bois Escapes kan niet verantwoordelijk worden gehouden voor ongevallen buiten het cottage. Huurders zijn verantwoordelijk voor hun eigen veiligheid tijdens buitenactiviteiten." })}</p>
      </div>
    </div>
  );
}
